                        ...
                        , const uint8_t *row, uint32_t row_size)
{
  ...
  {
    //步骤1，进行行比较
    row_to_match = cfl_cursor_row_get(cursor_);
...
ha_cfl::locate_next2(bool &over)
{
  ...
  */
  ...
  {
    /*情况1处理*/
    DBUG_ASSERT(cfl_cursor_page_get(cursor_) != NULL);
    cfl_cursor_position_set(cursor_, 1);
    cfl_cursor_fill_row(cursor_);
    return 0;
  }
  else if (cfl_cursor_position_get(cursor_) == CFL_CURSOR_AFTER_END)
  {
                              ...
                              , bool &over)
{
  ...
  */
  ...
  if (row_no >= row_count)
  {
    ...
    page_no++;
    row_no = 0;
    page = CflPageManager::GetPage(table->GetStorage(), page_no);
    //后续页是否存在
    if (page == NULL)
    {
      //情况2-2
...
{
  cfl_dti_t rowkey;
  uint8_t *row = cfl_cursor_row_get(cursor_);
  rowkey = cfl_row_get_key_data(table->field, row);
